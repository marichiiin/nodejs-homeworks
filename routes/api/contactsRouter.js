...
router.post('/', async (req, res, next) => {
  ...
  // If everythin is fine with body, add a unique identifier to the contact object
...
router.delete('/:contactId', async (req, res, next) => {
  // Gets the id parameter
  // Calss the removeContact function to work with the JSON file contacts.json
  // If thre is such an id, it returns JSON of the format {"message": "Not found" and status 404}
  try {
    const { contactId } = req.params;
    const result = await removeContact(contactId);

    if (!result) {
      res.status(404).json({ message: "Not found (removeContact)" });
...
router.put('/:contactId', async (req, res, next) => {
  ...
  // If  everything is fine wtih body, call the updateContact(contactId, body) function (write it) to update to update the contact in the contacts.json file
  ...
  try {
    ...
    if (!result) {
      res.status(404).json({ message: "Not found (updateContact)" });
